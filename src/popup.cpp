...
static void ok_event_cb(lv_event_t *e)
{
  ...
  lv_obj_t *tile_view = lv_obj_get_parent(tile);

  lv_obj_set_scroll_dir(tile_view, LV_DIR_HOR);
  lv_obj_del(overlay);
}

lv_obj_t *show_error_popup(lv_obj_t *parent_tile, const char *msg)
{
  lv_obj_t *tile_view = lv_obj_get_parent(parent_tile);
  lv_obj_set_scroll_dir(tile_view, LV_DIR_NONE);
...
lv_obj_t *show_fullscreen_popup(lv_obj_t *parent)
{
  ...
  lv_obj_remove_style_all(overlay);
  lv_obj_set_size(overlay, lv_disp_get_hor_res(nullptr), lv_disp_get_ver_res(nullptr));
  lv_obj_set_style_bg_color(overlay, BLACK, 0);
