...
Button::Button(lv_obj_t *parent_grid, const ButtonData &data, uint8_t grid_col,
  ...
  lv_obj_set_grid_cell(btn, LV_GRID_ALIGN_STRETCH, grid_col, 1,
...
void Button::eventHandler(lv_event_t *e) {
  ...
  if (code == LV_EVENT_PRESSED) {
    ...
    if (!long_press_handled) {
      ...
      lv_opa_t ratio = elapsed >= LONG_PRESS_DURATION
                           ? 255
                           : (255 * elapsed / LONG_PRESS_DURATION);
      lv_color_t base =
          toggleable ? (toggled ? color_on : color_off) : color_off;
