...
VoiceTile::VoiceTile(lv_obj_t *tileview, int row_id,
  ...
  static lv_coord_t col_dsc[] = {COLUMN_WIDTH, CENTER_WIDTH, COLUMN_WIDTH,
                                 LV_GRID_TEMPLATE_LAST};
  static lv_coord_t row_dsc[] = {VISUALISER_HEIGHT, BUTTON_HEIGHT,
  ...
  for (int side = 0; side < 2; ++side) {
    ...
    for (int i = 0; i < 4; ++i) {
      indicators[side * 4 + i] =
          new Indicator(::indicators[side * 4 + i], column);
    }
  }
